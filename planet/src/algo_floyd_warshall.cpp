...
/*
new discoveries after quadratic:
vertices iterators in single loops: important to not iterate on all vertices but only n first vertices, not nscc added vertices
n2 dijkstra and floyd warshall are both bad.
still too slow :-(
*/ 
...
void planet_delivery() {
    ...
    DistanceMatrix distances(n+nscc);
    ...
    int sink_index = boost::get(boost::vertex_index, G, *(it1_end-nscc-1)); // last vertex added before adding nscc vertices
    for(; it1_begin!=it1_end-nscc; ++it1_begin) {
        int source_index = boost::get(boost::vertex_index, G, *it1_begin);
        if (source_index < k) { // source is a warehouse
            if (dm[source_index][sink_index] < shortest_path) {
                shortest_path = dm[source_index][sink_index];
            }
        }
