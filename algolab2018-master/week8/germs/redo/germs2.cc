...
double rad_to_time(double r){
	return std::ceil(std::sqrt(std::sqrt(r/4) - 0.5));
}
...
bool testcase(){
	...
	for(int i=0; i<n; i++){
		min_distances[i] = CGAL::min(min_distances[i], 4*(l - bacteries[i].first.x())*(l - bacteries[i].first.x()));
		min_distances[i] = CGAL::min(min_distances[i], 4*(r - bacteries[i].first.x())*(r - bacteries[i].first.x()));
		min_distances[i] = CGAL::min(min_distances[i], 4*(t - bacteries[i].first.y())*(t - bacteries[i].first.y()));
		min_distances[i] = CGAL::min(min_distances[i], 4*(b - bacteries[i].first.y())*(b - bacteries[i].first.y()));
	}
	
	std::sort(min_distances.begin(), min_distances.end());
	// Output the results
	std::cout << rad_to_time(min_distances[0])
		<< " " << rad_to_time(min_distances[(min_distances.size()-1)/2 +(min_distances.size()+1)%2])
